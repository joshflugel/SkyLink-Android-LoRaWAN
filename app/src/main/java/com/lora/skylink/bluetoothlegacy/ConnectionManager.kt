...
import com.lora.skylink.common.logv
import com.lora.skylink.common.logw
...
object ConnectionManager {
    ...
    fun registerListener(listener: ConnectionEventListener) {
        ...
        logd("Added listener $listener" + ", ${listeners.size} listeners total")
    }
    ...
    fun unregisterListener(listener: ConnectionEventListener) {
        ...
        toRemove?.let {
            listeners.remove(it)
            logd("Removed listener ${it.get()}, ${listeners.size} listeners total")
        }
    }

    fun connect(device: BluetoothDevice, context: Context?) {
        if (device.isConnected()) {
            loge("Already connected to ${device.address}!")
        } else {
    ...
    fun readCharacteristic(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() && characteristic.isReadable()) {
            ...
            loge("Attempting to read ${characteristic.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            loge("Not connected to ${device.address}, cannot perform characteristic read")
        }
    }
    ...
    fun writeCharacteristic(
        ...
        val writeType = when {
            ...
            characteristic.isWritableWithoutResponse() -> {
                ...
                loge("Characteristic ${characteristic.uuid} cannot be written to")
                return
            }
        }
        if (device.isConnected()) {
            ...
            loge("Not connected to ${device.address}, cannot perform characteristic write")
        }
    }
    ...
    fun readDescriptor(device: BluetoothDevice, descriptor: BluetoothGattDescriptor) {
        if (device.isConnected() && descriptor.isReadable()) {
            ...
            loge("Attempting to read ${descriptor.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            loge("Not connected to ${device.address}, cannot perform descriptor read")
        }
    }
    ...
    fun writeDescriptor(
        ...
        if (device.isConnected() && (descriptor.isWritable() || descriptor.isCccd())) {
            loge("Enqueueing Operation: WRITE DESCRIPTOR")
            enqueueOperation(DescriptorWrite(device, descriptor.uuid, payload))
        } else if (!device.isConnected()) {
            loge("Not connected to ${device.address}, cannot perform descriptor write")
        } else if (!descriptor.isWritable() && !descriptor.isCccd()) {
            loge("Descriptor ${descriptor.uuid} cannot be written to")
        }
    }
    ...
    fun enableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            loge("Not connected to ${device.address}, cannot enable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            loge("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun disableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            loge("Not connected to ${device.address}, cannot disable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            loge("Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun requestMtu(device: BluetoothDevice, mtu: Int) {
        if (device.isConnected()) {
            ...
            loge("Not connected to ${device.address}, cannot request MTU update!")
        }
    }
    ...
    private fun doNextOperation() {
        if (pendingOperation != null) {
            loge("doNextOperation() called when an operation is pending! Aborting.")
            return
        }

        val operation = operationQueue.poll() ?: run {
            logv("Operation queue empty, returning")
            return
        }
        ...
        if (operation is Connect) {
            with(operation) {
                logw("Connecting to ${device.address}")
                device.connectGatt(context, false, mGattCallback, TRANSPORT_LE)
            }
            return
        }
    ...
    private val mGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    logw("onConnectionStateChange: connected to $deviceAddress, $deviceName")
                ...
                loge("INSERT TOAST HERE, DEVICE BUSY or UNAVAILABLE")
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    logw("Discovered ${services.size} services for ${device.address}.")
                    printGattTable()
        ...
        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            logw("ATT MTU changed to $mtu, success: ${status == BluetoothGatt.GATT_SUCCESS}")
            ...
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        logi("Read characteristic $uuid | value: ${value.toHexString()}")
                        listeners.forEach { it.get()?.onCharacteristicRead?.invoke(gatt.device, this) }
                    }
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        loge("Read not permitted for $uuid!")
                    }
                    else -> {
                        loge("Characteristic read failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onCharacteristicWrite(
            ...
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        logi("Wrote to characteristic $uuid | value: ${value.toHexString()}")
                        listeners.forEach { it.get()?.onCharacteristicWrite?.invoke(gatt.device, this) }
                    }
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        loge("Write not permitted for $uuid!")
                    }
                    else -> {
                        loge("Characteristic write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onCharacteristicChanged(
            ...
            with(characteristic) {
                logi("Characteristic $uuid changed | value: ${value.toHexString()}")
        ...
        override fun onDescriptorRead(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        logi("Read descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        ...
                        loge("Descriptor read failed for $uuid, error: $status")
                    }
                }
            }
        ...
        override fun onDescriptorWrite(
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        logi("Wrote to descriptor $uuid | value: ${value.toHexString()}")
                    ...
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        ...
                        loge("Descriptor write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        private fun onCccdWrite(
            ...
            when {
                notificationsEnabled -> {
                    logw("Notifications or indications ENABLED on $charUuid")
                    listeners.forEach {
                ...
                notificationsDisabled -> {
                    logw("Notifications or indications DISABLED on $charUuid")
                    listeners.forEach {
                    ...
                    loge("Unexpected value ${value.toHexString()} on CCCD of $charUuid")
                }
            }
        }
    }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            with(intent) {
                if (action == BluetoothDevice.ACTION_BOND_STATE_CHANGED) {
                    ...
                    logw("${device?.address} bond state changed | $bondTransition")
                }
            }
        }
